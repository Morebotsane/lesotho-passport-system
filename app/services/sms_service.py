...
# Optional: Keep a synchronous version for backwards compatibility or testing
class SMSServiceSync(SMSService):
    """
    Synchronous version of SMS service for backwards compatibility.
    Use SMSService (async) for production!
    """
    ...
    def _queue_sms(
        ...
        """
        Synchronous SMS sending (OLD BEHAVIOR).
        Only use for testing or specific edge cases!
        """
        ...
        try:
            ...
            notification.twilio_message_sid = twilio_message.sid
            notification.status = NotificationStatus.SENT
            notification.sent_at = datetime.utcnow()
            
            logger.info(f"SMS sent synchronously: {twilio_message.sid}")
